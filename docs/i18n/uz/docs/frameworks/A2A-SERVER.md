...
title: "OmniRoute A2A serveri hujjatlari"
version: 3.8.40
lastUpdated: 2026-06-28
---

# OmniRoute A2A serveri hujjatlari

> Agentlararo protokol v0.3 — OmniRoute aqlli yoʻnaltirish agenti sifatida

A2A interfeysi ikki ko‘rinishga ega:

- **JSON-RPC 2.0** — `POST /a2a` manzilida (asosiy kirish nuqtasi, `src/app/a2a/route.ts` faylida belgilangan).
- **REST** — boshqaruv panellari va vositalar uchun `/api/a2a/*` ostida (holat, vazifalar roʻyxati, bekor qilish).

Vazifalar `A2ATaskManager` orqali kuzatib boriladi (`src/lib/a2a/taskManager.ts`, standart TTL — 5 daqiqa). Koʻnikmalar `src/lib/a2a/taskExecution.ts` ichidagi `A2A_SKILL_HANDLERS` orqali yoʻnaltiriladi.
