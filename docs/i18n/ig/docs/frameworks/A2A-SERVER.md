...
title: "Akwụkwọ Nkọwa Sava OmniRoute A2A"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Akwụkwọ Nkọwa Sava OmniRoute A2A

> Usoro Nkwukọrịta Onye Nnọchi Anya-na-Onye Nnọchi Anya v0.3 — OmniRoute dị ka onye nnọchi anya nwere ọgụgụ isi maka ịhọrọ ụzọ

Ọdịdị A2A nwere akụkụ abụọ:

- **JSON-RPC 2.0** na `POST /a2a` (ebe mbata izizi, nke akọwapụtara na `src/app/a2a/route.ts`).
- **REST** n'okpuru `/api/a2a/*` maka dashboard na ngwa ọrụ (ọnọdụ, ndepụta ọrụ, ịkagbu).

`A2ATaskManager` (`src/lib/a2a/taskManager.ts`, TTL ndabara nke nkeji 5) na-esochi ọrụ. A na-eziga nka site na `A2A_SKILL_HANDLERS` dị na `src/lib/a2a/taskExecution.ts`.
