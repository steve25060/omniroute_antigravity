...
title: "Takardun Sabar OmniRoute A2A"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Takardun Sabar OmniRoute A2A

> Yarjejeniyar Agent-zuwa-Agent v0.3 — OmniRoute a matsayin wakilin kai hanya mai basira

Fagen A2A yana da hanyoyi biyu:

- **JSON-RPC 2.0** a `POST /a2a` (babbar mashigar hukuma, wadda aka ayyana a `src/app/a2a/route.ts`).
- **REST** ƙarƙashin `/api/a2a/*` don allunan sa ido da kayan aiki (matsayi, jerin ayyuka, sokewa).

Ana bibiyar ayyuka ta `A2ATaskManager` (`src/lib/a2a/taskManager.ts`, tsoffin mintuna 5 na TTL). Ana aika ƙwarewa ta `A2A_SKILL_HANDLERS` a cikin `src/lib/a2a/taskExecution.ts`.
