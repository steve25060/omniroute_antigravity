...
title: "Ìwé Àkọsílẹ̀ Olùpèsè A2A OmniRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Ìwé Àkọsílẹ̀ Olùpèsè A2A OmniRoute

> Ìlànà Agent-to-Agent v0.3 — OmniRoute gẹ́gẹ́ bí aṣojú ọlọ́gbọ́n fún ìdarí ipa

Ààyè ìbáṣepọ̀ A2A ní ojú méjì:

- **JSON-RPC 2.0** ní `POST /a2a` (ojú-ọ̀nà ìwọlé àkọ́kọ́, tí a ṣàlàyé rẹ̀ nínú `src/app/a2a/route.ts`).
- **REST** lábẹ́ `/api/a2a/*` fún àwọn pánẹ́ẹ̀lì ìṣàkóso àti irinṣẹ́ (ipò, àtòjọ iṣẹ́, ìfagilé).

`A2ATaskManager` (`src/lib/a2a/taskManager.ts`, TTL ìṣẹ́jú márùn-ún ní àìṣe) ló ń tọpinpin àwọn iṣẹ́. A ń pín àwọn ọgbọ́n iṣẹ́ nípasẹ̀ `A2A_SKILL_HANDLERS` nínú `src/lib/a2a/taskExecution.ts`.
