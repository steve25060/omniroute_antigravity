 ...
 *
 * Usage:
 *   node scripts/i18n/check-key-completeness.mjs            # blocking
...
async function readCatalogs() {
  const files = (await fs.readdir(MESSAGES_DIR)).filter((f) => f.endsWith(".json")).sort();
  const locales = {};
  let en = null;
  for (const file of files) {
    const code = file.slice(0, -".json".length);
    const parsed = JSON.parse(await fs.readFile(path.join(MESSAGES_DIR, file), "utf8"));
    if (code === SOURCE_LOCALE) en = parsed;
    else locales[code] = parsed;
  }
  if (!en) throw new Error(`[i18n-keys] ${SOURCE_LOCALE}.json not found in ${MESSAGES_DIR}`);
  return { en, locales };
}
...
async function main() {
  ...
  console.error(
    `[i18n-keys] ${warnOnly ? "WARN" : "FAIL"} — ${gaps.length}/${count} locales differ from en.json (${missingTotal} missing, ${extraTotal} extra leaves):`
  );
  console.error(formatReport(gaps));
  console.error(
    "[i18n-keys] Fix: node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers (adds the missing keys and translates them); extra keys mean the source dropped them — remove them from the locale."
  );
