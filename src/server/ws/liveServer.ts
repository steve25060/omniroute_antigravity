...
function extractQueryToken(request: import("http").IncomingMessage): string | null {
  try {
...
async function authorizeConnection(request: import("http").IncomingMessage): Promise<WsAuthResult> {
  ...
  const token =
    extractBearerToken(request) || extractAltTokenHeader(request) || extractQueryToken(request);
  ...
  if (!token) {
    ...
    // Loopback carve-out: allow loopback clients connecting from allowed loopback origins
    // (e.g. the local dashboard running on 127.0.0.1:20128 or localhost:20128)
    // so local users and unauthenticated dashboard tabs don't enter an infinite disconnect loop.
    ...
    const isLoopbackRemote =
      ...
      remote === "localhost";
    if (isLoopbackRemote && isOriginAllowed(originStr)) {
      return { authorized: true, sessionId };
    }
